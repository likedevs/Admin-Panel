                            ...
                            <select name="prop[{{ $property->id }}]" class="form-control">
                                @if (!empty($property->multidata)))
                                    @foreach ($property->multidata as $key => $multidata)
                                        ...
                                        <option value="{{ $value->value }}" {{ getPropertiesData(Request::segment(3), $property->id, 0) ==  $value->value ? 'selected' : ''  }}>{{ $value->name}} {{ $property->translationByLanguage($lang->id)->first()->unit }}</option>
